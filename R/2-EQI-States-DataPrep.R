...
files = list.files(path = 'data/denue_entidades/raw', full.names = TRUE)
...
for (file in files) {
  read_csv(file) %>% 
    mutate(
      ...
      nom_estab = ifelse(is.na(nom_estab), raz_social, nom_estab)
    ) %>% 
    # Transmute to dataset
    transmute(
      # Geographic and political and bussiness ids
      cve_ent, cve_mun, id,
      ...
      # Months open
      # month_open = as.numeric(ym('2022-04') - ym(fecha_alta))/30,
      # Fiscal type dummies
    ...
    write_excel_csv(
      paste0('data/denue_entidades/model_input/', str_extract(file, '\\d{2}'), '_112022.csv')
      )
